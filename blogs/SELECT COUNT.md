### [不就是SELECT COUNT语句吗，竟然能被面试官虐的体无完肤](https://www.cnblogs.com/hollischuang/p/11711778.html)
...
关于COUNT函数，在[MySQL官网](https://dev.mysql.com/doc/refman/8.0/en/group-by-functions.html#function_count)中有详细介绍：
...
> 【强制】不要使用count(列名)或count(常量)来替代count(*),count(*)是SQL92定义的标准统计行数的语法，跟数据库无关，跟NULL和非NULL无关。<br/>
说明: count(*) 会统计值为NULL的行，而count(列名)不会统计此列为NULL的值。
